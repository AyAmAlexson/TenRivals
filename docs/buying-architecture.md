...
`brand`, `model_name`, `generation`, `variant`, `category` (choices: racquet / shoes / string_reel / string_set / overgrip / balls / apparel / bag / accessory), `gender`, `court`, `size`, `size_system`, `grip_size`, `color`, `color_policy` (required / preferred / optional), `weight_g`, `head_size`, `string_pattern`, `length_cm`, `required_attributes` JSON, `optional_attributes` JSON, `uncertainties` JSON, `manufacturer_code`, `ean`, `upc`, `aliases` JSON, `field_sources` JSON (`client` / `ai` / `canonical` / `staff` per field), `enrichment_snapshot` JSON (копия `RacquetSpecification` на момент резолва — исторические запросы не переписываются при правке KB), `raw_ai_output` JSON, `prompt_version`, `model_version`, `edited_by_staff` bool, `canonical_product` FK nullable → `CanonicalProduct`.

**Поток нормализации ракеток:** free-form → AI normalize → canonical racquet resolution (`RacquetSpecification`) → tennis specification enrichment → staff review → supplier search. Enrichment не опирается на «память» LLM: спеки берутся из поддерживаемой таблицы. При неоднозначности (например `Pure Drive 2025` без head size / variant) вес и head size не угадываются — остаются в `uncertainties`. Matching использует обогащённые поля как hard constraints (head / weight / pattern / generation / Team|Lite|Tour|Plus|Junior / grip).

**`RacquetSpecification`** — поддерживаемая KB конкретной ракетки (семейство + generation + variant + head + unstrung weight + pattern + length + aliases + optional FK → `CanonicalProduct`). Сид: `manage.py seed_racquet_specs`.
