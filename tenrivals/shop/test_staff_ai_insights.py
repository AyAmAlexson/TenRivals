...
class InsightPayloadTests(TestCase):
    ...
    def test_analytics_payload_includes_required_windows(self):
        ...
        self.assertIn('net_proceeds', month)
        self.assertIn('gross_profit', month)
        self.assertNotIn('income', month)
        self.assertTrue(
    ...
    def test_orders_csv_is_line_level_and_honors_date_filter(self):
        ...
        self.assertIn('order_net_proceeds', body)
        self.assertIn('order_gross_profit', body)
        self.assertNotIn('order_income', body)
...
class InsightViewTests(TestCase):
    def setUp(self):
        ...
        self.analytics_url = reverse('administration:staff_sales_analytics')
    ...
    def test_staff_downloads_markdown_pack(self):
        ...
        self.assertContains(finance, 'Net Proceeds')
        self.assertContains(finance, 'Gross Profit')
        self.assertContains(finance, 'This month')
        analytics = self.client.get(self.analytics_url)
        self.assertEqual(analytics.status_code, 200)
        self.assertContains(analytics, 'Net Proceeds')
        self.assertContains(analytics, 'Gross Profit')
        self.assertContains(analytics, 'Avg check')
