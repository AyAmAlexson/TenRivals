...
worker: celery -A tenrivals.celery:app worker --loglevel=info

# Celery Beat для планирования периодических задач (использует базу данных)
beat: celery -A tenrivals.celery:app beat --loglevel=info --scheduler django_celery_beat.schedulers:DatabaseScheduler
